...
type BarCodeGenerationReturn = {
    barCode: string
    amount: string
    expirationDate: string
}
export class BarCodeGeneration {
    execute(code: string): BarCodeGenerationReturn {
        if (code.length === 47) return this.bankBoletoFlux(code)
        return this.concessionaryBoletoFlux(code)
    }

    bankBoletoFlux(code: string): BarCodeGenerationReturn {
        const [firstField, secondField, thirdField, fourthField, fifthField] = BoletoHelpers.splitBankBoleto(code)
        ...
        const rightCode = fifthField + charIndex20To24 + charIndex25To34 + charIndex35To44

        const verificationCode = BoletoHelpers.generateBankBarcodeVerificationDigit(leftCode + rightCode)

        if (verificationCode !== parseInt(fourthField, 10))
            throw new BadRequest({ data: { message: 'Failed to verify barCode automate verification digit' } })

        return {
            barCode: leftCode + verificationCode + rightCode,
            amount: BoletoHelpers.parseValue(value),
            expirationDate: BoletoHelpers.parseExpirationDate(parseInt(expirationCode, 10))
        }
    }

    concessionaryBoletoFlux(code: string): BarCodeGenerationReturn {
        const [firstField, secondField, thirdField, fourthField] = BoletoHelpers.splitConcessionaryBoleto(code)
        ...
        const barCode = charIndex1To11 + charIndex12To22 + charIndex23To33 + charIndex34To44

        const [lefCode] = barCode.match(/^\d{3}/)
        const [verificationCode] = barCode.match(/(?<=^\d{3})\d/)
        const [rightCode] = barCode.match(/(?<=^\d{4})\d+/)

        const generatedVerifyDigit = BoletoHelpers.generateVerifyDigit(lefCode + rightCode)
        ...
        const [expirationDate] =
            segment === '6' ? barCode.match(/(?<=^\d{13})\d{8}/) : barCode.match(/(?<=^\d{19})\d{8}/)

        return {
            barCode,
            amount: BoletoHelpers.parseValue(value),
            expirationDate: BoletoHelpers.verifyConcessionaryExpirationDate(expirationDate)
        }
    }
}
