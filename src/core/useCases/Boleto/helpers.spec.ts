...
describe('Test helpers to parse a boleto', () => {
    it('generateVerifyDigit should return true if the verifyDigit is right', () => {
        ...
        expect(BoletoHelpers.generateVerifyDigit('846' + '0000001435900240200240500024384221010811')).toBe(7)
    ...
    it("verifyFieldWithDigit should throw an erro if the verifyDigit ins't right", () => {
        BoletoHelpers.verifyFieldWithDigit({ code: '84670000001', digit: 7, field: 'fisrt' })
        const err = () => BoletoHelpers.verifyFieldWithDigit({ code: '84670000001', digit: 6, field: 'fisrt' })
        expect(err).toThrow(BadRequest)
    })
    ...
    it('splitBankBoleto should return 5 fileds', () => {
        const [firstField, secondField, thirdField, fourthField, fifthField] = BoletoHelpers.splitBankBoleto(
    ...
    it('splitConcessionaryBoleto should return 4 fields', () => {
        expect(BoletoHelpers.splitConcessionaryBoleto('846700000017435900240209024050002435842210108119').length).toBe(
            4
        )
    })
    ...
    it('splitConcessionaryBoleto should return 4 fields', () => {
        expect(
            BoletoHelpers.generateBankBarcodeVerificationDigit('2129' + '758700000020000001121100012100447561740')
        ).toBe(9)
    })
    ...
    it('verifyConcessionaryExpirationDate should return ISO date value', () => {
        expect(BoletoHelpers.verifyConcessionaryExpirationDate('20000703')).toBe('2000-07-03')
        expect(BoletoHelpers.verifyConcessionaryExpirationDate('20002201')).toBe('Barcode has no expiration date')
        expect(BoletoHelpers.verifyConcessionaryExpirationDate('20000231')).toBe('Barcode has no expiration date')
    })
})
